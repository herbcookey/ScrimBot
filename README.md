...
- `/내전 생성 제목 [게임] [방식] [1지망] [2지망] [3지망]`: 현재 채널에 모집 패널을 만듭니다. LoL은 1·2지망이 필요합니다.
...
- `/내전 mmr설정 사용자 라인 [티어] [세부단계] [점수] [게임]`: 현재 시즌 라인 MMR을 설정합니다. 서버 관리 권한이 필요합니다. Discord 명령 이름 제한 때문에 `MMR설정`이 아니라 소문자 `mmr설정`입니다.
...
2. 관리자가 `/내전 mmr설정`으로 참가자별 지망 라인 점수 입력
3. `/내전 생성`에서 방식과 생성자 지망 입력

아이언~다이아는 티어와 1~4 단계를 받습니다. 마스터·그랜드마스터는 세부단계에 `하`, `중`, `상`을 넣습니다. 챌린저는 세부단계가 필요 없습니다. 기존 점수 이전이 필요하면 티어 대신 `점수`를 직접 넣어도 됩니다. 승패나 기존 경기 기록은 이 명령에서 안 건드립니다.
...
종료나 취소 뒤 기본 10분이 지나면 DB에 저장된 동적 채널 ID만 삭제합니다. 한쪽만 삭제되면 성공한 ID만 비우고 실패한 쪽은 다시 시도합니다. 이름이 비슷한 사용자 채널은 삭제하지 않습니다.
...
1차부터 3A까지는 `20260826012146_schema.sql`, 3B는 `20260826025650_phase_3b_role_mmr.sql`, 동적 보이스는 `20260826034441_phase_3b_dynamic_voice_channels.sql`입니다. 초기화한 DB에서는 프로젝트 루트에서 `supabase db push` 한 번이면 순서대로 들어갑니다. 3B는 LoL의 라인 기능만 켜고 기존 글로벌 점수를 다섯 라인에 복사하지 않습니다. 끝난 경기도 다시 계산 안 합니다. migration 전에 만들어진 경기는 경기별 라인 기능 값이 `false`라서 진행 중인 기존 경기도 3A 방식으로 마무리됩니다.
...
개발 의존성을 설치한 가상 환경에서 실행합니다. Discord 쪽은 오프라인 테스트이고, `tests/test_matches.py`, `tests/test_role_matches.py`, `tests/test_dynamic_voice_matches.py`는 실제 PostgreSQL 통합 테스트입니다.
