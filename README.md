...
현재 버전은 **1.1.0**입니다. 버전별 변경 사항은 [내전봇 패치 내역](CHANGELOG.md)에서 확인할 수 있습니다.
...
- `/내전 생성 제목 1지망 [2지망] [3지망] [모집시간] [게임] [방식]`: 현재 채널에 모집 패널을 만듭니다. 생성자의 1지망은 필수이고 2·3지망은 선택입니다. 3지망을 입력하려면 2지망도 입력해야 하며 제출한 지망은 서로 달라야 합니다.
...
- `/내전 기록 [사용자:@사용자] [게임] [시즌] [페이지]`: 종료 경기 목록을 5개씩 조회하고 당시 팀·라인·승패와 MMR 변동을 확인합니다. 예: `/내전 기록 사용자:@사용자 게임:lol 페이지:2`.
- `/내전 경기조회 경기번호:<양의 정수>`: 현재 서버의 종료 경기 팀 구성과 참가자별 MMR 변동을 조회합니다. 진행 중인 경기는 현재 모집 패널을 안내합니다.
...
- `/내전 라인변경 1지망 [2지망] [3지망]`: 모집 또는 준비 확인 중 지망을 바꿉니다. 3지망은 2지망 입력 시에만 사용할 수 있습니다.
...
- `/내전 기록 [사용자] [게임] [시즌] [페이지]`는 생략 시 실행자·`lol`·1페이지를 사용하며 페이지당 5경기를 보여 줍니다. 라인 MMR 게임은 시즌 생략 시 활성 시즌, 일반 MMR 게임은 전체 시즌을 조회합니다. 모집 패널에만 있던 대기자와 결과가 없는 경기·취소 경기는 제외합니다.
- `/내전 경기조회 경기번호`는 현재 서버의 경기만 찾고 종료 결과가 있는 경기의 A/B팀, 배정 라인과 이력 테이블에 저장된 경기 전후 MMR을 표시합니다. 과거 점수를 현재 MMR로 대신 계산하지 않습니다.
...
LoL은 탑, 정글, 미드, 원딜, 서폿 점수를 따로 씁니다. 라인 row가 없으면 미배치고 실제 0점과는 다른 상태입니다. 참가할 때 낸 지망 라인은 전부 배치돼 있어야 합니다. 1지망만 제출하거나 2·3지망을 추가할 수 있고, 3지망은 2지망이 있을 때만 제출할 수 있습니다. 평균은 0보다 큰 배치 점수만 더해서 단순 평균을 냅니다.

처음 쓰는 서버는 아래 순서로 하면 됩니다.

1. `/내전 시즌시작 이름:시즌 1` 실행
2. 각 사용자가 `/내전 등록`으로 자신의 지망 라인 MMR을 등록하거나 관리자가 `/내전 mmr설정`으로 입력
3. `/내전 생성`에서 방식과 생성자 지망 입력(1지망 필수, 2·3지망 선택)
